...
class AzureBlobUploader:
    ...
    def __init__(self, config: AzureConfig, local_storage_path: Path):
        ...
        if config.blob_endpoint and config.sas_token:
            ...
            match = re.search(
                r"https://([^.]+)\.blob\.core\.windows\.net", config.blob_endpoint
            )
            if match:
            ...
            container_url = (
                f"{config.blob_endpoint}/{config.container_name}?{sas_token}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Connected to Azure container '{config.container_name}' using SAS token"
                )
        ...
        # Fall back to old method using storage_account and storage_key
            ...
            logger.warning(
                "Azure credentials not configured - uploads will be disabled"
            )
            self.blob_service_client = None
            self.container_client = None
        ...
        try:
            ...
            if not container_client.exists():
                ...
                try:
                    ...
                    logger.info(
                        f"Created Azure container: {self.config.container_name}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Could not create container (might already exist or insufficient permissions): {e}"
                    )
            else:
    ...
    def file_needs_upload(self, local_file: Path, blob_path: str) -> bool:
        ...
        try:
            blob_client = self.blob_service_client.get_blob_client(
                container=self.config.container_name, blob=blob_path
            )
    ...
    def upload_file(self, local_file: Path) -> Dict[str, any]:
        ...
        try:
            ...
            for attempt in range(self.config.max_retries):
                try:
                    ...
                    if blob_client.exists():
                        ...
                        logger.info(
                            f"Successfully uploaded: {blob_path} ({file_size_mb:.2f} MB)"
                        )
                    ...
                    if attempt < self.config.max_retries - 1:
                        ...
                        logger.warning(
                            f"Upload attempt {attempt + 1} failed for {blob_path}, retrying in {wait_time}s: {e}"
                        )
            ...
            logger.error(
                f"Failed to upload {blob_path} after {self.config.max_retries} attempts: {last_error}"
            )
            return {
                "local_file": str(local_file),
                "blob_path": blob_path,
    ...
    def upload_files_parallel(self, files: List[Path]) -> Dict[str, List]:
        ...
        logger.info(
            f"Starting upload of {total_files} files with {self.config.max_workers} workers"
        )
            ...
            for future in as_completed(future_to_file):
                try:
                    ...
                    if completed % 10 == 0 or completed == total_files:
                        logger.info(
                            f"Upload progress: {completed}/{total_files} files processed"
                        )

                    if status == "uploaded":
                        logger.debug(
                            f"Uploaded: {result['blob_path']} ({result['size_mb']:.2f} MB)"
                        )
                    elif status == "failed":
                        logger.error(
        ...
        logger.info(
            f"Upload complete - Uploaded: {len(results['uploaded'])}, Skipped: {len(results['skipped'])}, Failed: {len(results['failed'])}"
        )
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            "Azure connection test successful (limited permissions)"
                        )
                    except Exception as e2:
                        logger.warning(
                            f"Azure connection test completed with warnings: {e2}"
                        )
