        ...
        try:
            ...
            if storage_manager:
                try:
                    ...
                    components["storage"] = {"healthy": True, "stats": storage_stats}
    ...
    async def trigger_upload(background_tasks: BackgroundTasks) -> Dict:
        ...
        try:
            ...
            if not storage_manager:
                raise HTTPException(
                    status_code=503, detail="Storage manager not available"
                )
    ...
    async def trigger_cleanup(background_tasks: BackgroundTasks) -> Dict:
        ...
        try:
            if not cleanup_service:
                raise HTTPException(
                    status_code=503, detail="Cleanup service not available"
                )

            # Trigger cleanup in background
            background_tasks.add_task(run_cleanup_task)

            return {"status": "started", "message": "Cleanup started in background"}
...
async def run_upload_task(files):
    ...
    try:
        ...
        logger.info(
            f"Background upload completed: {len(results['uploaded'])} uploaded, {len(results['failed'])} failed"
        )
